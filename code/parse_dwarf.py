...
class DwarfParser():
    def __init__(self, bin_path):
        ...
        logger.D(f"Read dwarf info from file -> {bin_path}")
    ...
    def bin2func(self, target_addr):
        ...
        for CU in self.dwarfinfo.iter_CUs():
            ...
            try:
                ...
                logger.D(f"Warning: Cannot find the DW_AT_low_pc & DW_AT_high_pc attributes!"
                    f"\n{top_die.__str__()}")
            else:
        ...
        for die in target_cu.iter_DIEs():
            if die.tag == 'DW_TAG_subprogram':
                try:
                    ...
                    logger.D(f"Warning: Cannot find the DW_AT_low_pc & "
        ...
        logger.I(f"The address <{target_addr}> can be found below:\nfile: {file_path}\n"
        ...
        logger.I(f'The starting line of function after <{func_name}>: {next_func_decl_line}')
        last_curly_bracket_line = find_end_curly_bracket(
            file_path, func_decl_line, next_func_decl_line)
        logger.I(f'The ending line of function <{func_name}>: {last_curly_bracket_line}')
    ...
    def get_main_addr(self):
        ...
        logger.I(f"Here is the main function in the source -> {src_filepath}: {main_line}")

        flineNumberDict = {}
        main_addr, fileBoundRangesList, fileBoundIndexList = readELF(
            self.bin_path, flineNumberDict, main_line, src_filepath)
        logger.I(f"Here is the main address in binary -> {main_addr}")
