...
class SensMap(object):
    def __init__(self, seed_len, seed_trace_len, max_combination):
        ...
        logger.D(f"Max Combinations: {max_combination}")
        logger.D(f"Number of Mutation Idxes: {len(idx_list)}")
        logger.D(f"#Loc: {seed_trace_len}")
        logger.D(f"Size(seed): {seed_len}")
    ...
    def update_crash_map(self, mutate_idx, crash_collection):
        if len(crash_collection) == 2:
            logger.D(f"Update crash location sensitivity map! mutate id: {mutate_idx}")
    ...
    def select_mutate_idx(self):
        ...
        for comb_id in range(1, self.max_combination + 1):
            ...
            min_idx = max_idx
            if len(idx_range) > 0:
                logger.D(f"Select the mutation idx from {comb_id}-combination")
    ...
    def save_maps_to_disk(self, seed_trace_hash):
        ...
        logger.D("Start saving the sensitivity map -> {sensitivity_filepath}")
        info = {
        ...
        logger.D(f"Finish writing the sensitivity map -> {sensitivity_filepath}")
