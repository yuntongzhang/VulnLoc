...
def parse_args():
	...
	if len(set(detailed_config.keys()) & set(values.DefaultItems)) != len(values.DefaultItems):
		raise Exception(f"ERROR: Missing required attributes in config.ini -> Required attributes: {values.DefaultItems}")
...
def choose_seed():
	...
	for seed_no in range(seed_num):
		if values.SeedPool[seed_no][0] == False:
...
def real_concfuzz_loop(config_info, poc_trace, seed_len):
	...
	while True:
		...
		while True:
			...
			logging.debug(f"[R-{round_no}-{subround_no}] Select the mutate idx -> {mutate_idx}: {actual_mutate_loc}")
			sens_map.mark_mutate_idx(mutate_idx)
			# mutate inputs
			inputs = mutate_inputs(selected_seed, config_info['poc_fmt'], config_info['#mutation'], actual_mutate_loc)
			...
			for item in result_collection:
				...
				if trace_hash not in values.TraceHashCollection:
					...
					values.ConcentratedInputCounter = store_input(values.ConcentratedInputFolder, values.ConcentratedInputCounter, config_info, inputs[input_no])
			...
			logging.debug(f"#Diff: {len(diff_collection)}; #ExeResult: {len(crash_collection)}; #seed: {len(values.SeedPool)}")
			...
			if duration >= config_info['local_timeout']: # exit if timeouts
				logging.debug(f"[R-{round_no}-{subround_no}] Timeout locally! -> Duration: {duration} ({ctime} - {stime}) in seconds")
				break
			# check whether all the locations get explored or not.
			unexplore_loc_idx_list = np.where(np.asarray([len(item) for item in sens_map.loc_map()['value']]) == 0)[0]
			logging.debug(f"[R-{round_no}-{subround_no}] #(Unexplored Locs): {len(unexplore_loc_idx_list)}")
...
if __name__ == '__main__':
	...
	init_log(tag, verbose, config_info)
